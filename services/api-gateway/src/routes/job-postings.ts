 ...
 *
 * Tenant-scoped CRUD over `job_postings`. RBP via EMPLOYEES area.
 */
...
jobPostingsRouter.get(
  ...
  async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
      ...
      const { status, department, search, limit, offset } = parsed.data;
      ...
      const result = await withTenant(req.tenantId!, async (tx) => {
        ...
        let sql = `SELECT * FROM job_postings WHERE tenant_id = $1::uuid`;
        let idx = 2;
        if (status) {
          sql += ` AND status = $${idx}`;
          params.push(status);
        ...
        const totalRows = (await tx.$queryRawUnsafe(
          `SELECT COUNT(*)::int AS count FROM job_postings WHERE tenant_id = $1::uuid`,
          req.tenantId!
