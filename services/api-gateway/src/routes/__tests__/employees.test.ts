...
const econovaEmployees: FixtureEmployee[] = [
  { id: "aaaaaaaa-1111-1111-1111-111111111111", tenantId: ECONOVA_TENANT, firstName: "Laura", lastName: "Bertolini", email: "laura@econova.org" },
  { id: "aaaaaaaa-1111-1111-1111-111111111112", tenantId: ECONOVA_TENANT, firstName: "Marco", lastName: "Rossi", email: "marco@econova.org" },
];
...
vi.mock("../../db/pool.js", () => ({
  withTenant: vi.fn(async (tenantId: string, fn: (tx: unknown) => Promise<unknown>) => {
    const dataset =
      tenantId === ECONOVA_TENANT ? econovaEmployees
      : tenantId === RTL_BANK_TENANT ? rtlBankEmployees
      : [];
    const tx = {
      employees: {
...
let mockSession: { user?: { id?: string; tenantId?: string | null } } | null = null;
...
describe("GET /employees contract", () => {
  ...
  it("returns RTL Bank-only data when session.user.tenantId = RTL Bank — RLS isolation", async () => {
    mockSession = { user: { id: "u1", tenantId: RTL_BANK_TENANT } };
    const res = await request(buildApp()).get("/employees");
    expect(res.status).toBe(200);
  ...
  it("returns empty data + total=0 when tenantId is unknown", async () => {
    mockSession = { user: { id: "u1", tenantId: "99999999-9999-9999-9999-999999999999" } };
    const res = await request(buildApp()).get("/employees");
    expect(res.status).toBe(200);
  ...
  it("session tenantId takes precedence over X-Tenant-Id header (defense-in-depth)", async () => {
    ...
    const res = await request(buildApp())
      .get("/employees")
      .set("X-Tenant-Id", RTL_BANK_TENANT);
    expect(res.status).toBe(200);
