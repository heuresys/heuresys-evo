    ...
    try {
      ...
      const { result: rows } = await auditedTransaction(
        actor,
        {
          action: 'CREATE',
          category: 'SYSTEM',
          resourceType: 'workspace_templates',
          resourceId: 'pending',
          resourceName: `workspace-template:${d.name}`,
          newValue: d,
          metadata: { source: 'api-gateway:workspace.POST_templates' },
        },
        async (tx) => {
    ...
    try {
      ...
      const { result } = await auditedTransaction(
        actor,
        {
          ...
          resourceType: 'workspace_templates',
          resourceId: id,
          resourceName: `workspace-template:${id}`,
          oldValue: existing[0],
          newValue: parsed.data,
          metadata: { source: 'api-gateway:workspace.PATCH_templates' },
        },
        async (tx) => {
    ...
    try {
      ...
      await auditedTransaction(
        actor,
        {
          ...
          oldValue: existing[0],
          newValue: null,
          metadata: { source: 'api-gateway:workspace.DELETE_templates' },
        },
        async (tx) => {
          return tx.$queryRawUnsafe(
