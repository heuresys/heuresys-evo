...
> Updated: 2026-05-01 (RTG Phase 5 BLOCK 14 cutover-prep complete — STOP-AUTONOMO-FINAL pre-5.12 go/no-go decision) · See `PROJECT-STATE.md` for full state · See `PROJECT-LOG.md` for history

## Recent changes

**RTG Phase 5 BLOCK 14 cutover-prep complete** (~50 min, 2026-05-01): UX OK confermato Enzo, sblocco STOP-4. Prodotti 4 script bash (cross-system-transactions/perf-benchmark/dry-run-cutover/dr-drill-evo) + 3 doc cutover (rto-rollback-procedure/pre-cutover-checklist/user-comms-template). 5.5 cross-tx 10 transactions multi-tenant + RLS smoke, 5.6 perf-benchmark p95 evo/legacy con pass ≤1.2, 5.7 dry-run RTO ≤300s, 5.8 RTO+rollback runbook + risk register, 5.9 DR drill VM bare-metal target ≤30min, 5.10 30-item pre-cutover checklist A-E + sign-off, 5.11 6 user-comm templates italiano. Tag `rtg/evo/phase5/ready-for-go-no-go` da emettere. **STOP-AUTONOMO-FINAL** al task 5.12 go/no-go: richiede deployment effettivo + esecuzione 4 script + decisione Enzo GO/NO-GO/CONDITIONAL.

**RTG Phase 5 BLOCK 13 — deploy infra (5.1+5.2+5.3 + 5.4 prep)** (~50 min, 2026-05-01): **5.1** `infra/nginx/evo.heuresys.com.conf` (TLS port 443 via certbot, proxy / → :3200, /api/* → :8200 con prefix rewrite, /auth/ → :3200, /metrics deny, security headers HSTS+X-Frame+X-Content-Type, body limit 50M). **5.2** 3 systemd units in `infra/systemd/` (heuresys-api-gateway/app/enrichment) + `install-services.sh` installer. **DECISIONE: bare-metal systemd invece di Docker** per coerenza ADR-0001 (evo non ha Dockerfile, allineato con PostgreSQL bare-metal). Hardening NoNewPrivileges + PrivateTmp + ProtectSystem strict + ReadWritePaths /home/ubuntu/heuresys-evo. **5.3** `db/scripts/install-pull-cron.sh` cron daily 04:30 UTC pull `latest.dump` da bucket OCI (VM = read-only mirror, offset legacy backup 03:00 + push PC window). **5.4 prep**: `scripts/parallel-run-validate.sh` (~280L) — genera HTML+JSON report con reachability legacy/evo (api+fe), endpoint probes anonymous su 4 routes (employees/leaves/perf-reviews/audit-logs), 10-item manual UX checklist (login multi-tenant, leave flow, ESCO search, audit log, mobile, RLS isolation), performance baseline guidance. Dry-run testato (legacy 200, evo 000 pre-deploy). **STOP autonomo 4 raggiunto**: validation script ready, attesa Enzo per UX validation post-deploy.

**RTG Phase 4 task 4.14 retro — Phase 4 FULL CLOSE 100%** (~15 min, 2026-05-01): `.handoff/PHASE4-RETRO.md` full retro. **14/14 task done** (prima phase a chiusura completa). Effort 10-15gg estimate → ~3.5h actual single session BLOCK 9-12. +68 tests (12→93 api-gateway, 5x growth). 5 nuove route + 2 ADR + 1 migration. Lessons: schema discovery (baseline = SoT), pattern-reuse, honor-the-ADR ×2, vector cosine deferred safely. Tag `rtg/evo/phase4/done` ready. **STOP autonomo prima di BLOCK 13** Phase 5 deploy per check-in con Enzo (anche se DNS+OCI now unblocked).

**RTG Phase 4 tasks 4.11+4.12+4.13** (~30 min, 2026-05-01): 4.11 route `/esco/occupations/search` (ILIKE bilingue su `esco_occupations`, vector cosine deferred), 4.12 ADR-0017 + migration `223_tenant_schema_version_p0.sql` (append-only table, RLS, backfill v=1) + route `/admin/tenant-schema-version` GET history + POST /bump, 4.13 ADR-0018 Governance Audit Trail (riusa `audit_logs` + trigger baseline esistenti, no nuovo schema). ADR renumber 0010→0017 / 0011→0018 perché Cantiere B aveva già numerato 0010 RLS-coverage + 0011 test-coverage. 15 nuovi test (8 esco + 7 admin schema), 93 totali api-gateway, typecheck silent. BLOCK 11 task 4.14 retro = ultima task Phase 4.

**RTG Phase 4 task 4.10 endpoint port P0 (scope H Hybrid)** (~40 min, 2026-05-01): 3 nuove route api-gateway con RBP+RLS integration. `leaves.ts` (GET/POST/approve su `employee_time_off_requests`), `performance-reviews.ts` (GET list scope-aware), `audit-logs.ts` (GET filtered list). Pattern: `requireAuth` → `resolveTenant` → `cache.isAllowed()` → `getScopeCondition()` → `withTenant()` con $queryRawUnsafe parametrizzato (defense-in-depth: app scope + DB RLS). RBP cache singleton inizializzato in index.ts boot. **25 nuovi contract test** (13+5+7), 78 totali api-gateway, typecheck silent. Carry-forward 4.11 (ESCO KG verify), 4.12 (ADR-010 ontology versioning), 4.13 (ADR-011 audit trail triggers).

**RTG Phase 4 task 4.9 RLS scope helpers + coverage** (~15 min, 2026-05-01): `mergeScopedWhere(scopeCond, baseWhere)` in `db/pool.ts` (defense-in-depth: scope key override baseWhere su collision, deny-all sentinel preservato); aggiornato `withTenant()` doc post-baseline (605 RLS + 326 policies attivi). Script `services/api-gateway/scripts/check-rls-coverage.sh` wrapper su `db/scripts/rls-coverage.sql` (RTGB B2.3): exit-non-zero su FAIL rows. **6 nuovi test pool**, 53 test totali api-gateway, typecheck silent. RLS verify runtime delegato a primo testcontainers integration test in 4.10 (BLOCK 11) — ADR-0002 promote naturalmente lì.

**RTG Phase 4 task 4.8 RBP middleware port** (~30 min, 2026-05-01): `services/api-gateway/src/services/rbp-cache.ts` + `src/middleware/rbac.ts`. RBPCacheService (TTL 5min, factory pattern, $queryRawUnsafe sui 3 join rbp_roles+rbp_functional_areas+rbp_role_permissions); rbac.ts esporta `buildRequirePermission(cache)` (testabile via DI) + `requirePermission(area, action)` middleware (401/503/403 rejects) + `getScopeCondition(scope, ctx)` pure function (6 scope types con deny-all sentinel quando context manca). Schema RBP già completo nel baseline evo (18 rbp_* tables + 326 policies) — gap precedente sottostimato. **22 test passing** (10 cache + 12 middleware). Typecheck silent. Carry-forward 4.9 (RLS scope helpers + coverage check).

**STOP AUTONOMO 1-BIS — Frontend strategy brief** (~30 min, 2026-05-01): `docs/audits/frontend-strategy-brief.md`. Inventario legacy frontend: 231 pages, 11 dashboard, 27 widget (11 impl + 16 placeholder), 186 nav, 135 backend routes. 3 opzioni: **P** (Preserve 117gg fuori-scope-Q3), **R** (Rewrite 26gg feature-loss), **H** (Hybrid 41gg recommended — Q3 viable, P0 preserve + secondary redesign). Backend coverage mapping route→pagina per ogni opzione. Path comune pre-decisione: BLOCK 10 (4.8 RBP + 4.9 RLS + 4.11 + 4.12 + 4.13 backend, ~5.5-7gg). 4.10 endpoint port (BLOCK 11) attende decisione Enzo P/R/H.
