...
- Auth: NextAuth v4 Credentials, **8 canonical users** (`Heuresys2026!`) ristretti a `tests/.test-env` matrix (1 SUPERUSER `sysadmin` + 7 RTL Bank ruoli con username = work email canonica `firstname.lastname@rtl-bank.org` post-L50). `tenants.domain` SoT (econova.org · heuresys.com · rtl-bank.org · smartfood.org). 264 active employees · 265 active users · 1 platform user. Email canonical = `lower(strip-space-apostrophe(first.last))@<tenant.domain>`
